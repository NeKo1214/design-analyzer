...
export const fixMarkdownHeadings = (text: string): string => {
  ...
  // 3. 清洗单独成行的孤立 # 行（只有 # 符号和空白，无实质标题内容）
  result = result.replace(/^#{1,6}\s*$/gm, '');

  // 4. 清洗单独成行的 --- 分隔线，保留含 | 的表格分隔行
  result = result.replace(/^(?!\s*\|)-{3,}\s*$/gm, '');

  // 5. 统一换行符
  result = result.replace(/\r\n/g, '\n');

  // 6. 确保标题行前有空行（"# " 精确匹配，避免误处理行内 # 号）
  //    先处理"非换行字符 + 单个换行 + 标题行"
  result = result.replace(/([^\n])\n(#{1,6} )/g, '$1\n\n$2');
  //    再把所有"单个换行 + 标题行"都补成"双换行 + 标题行"
  result = result.replace(/\n(#{1,6} )/g, '\n\n$1');
...
export const markdownComponents = {
  ...
  em: ({ children }: { children?: ReactNode }) =>
    <span className="font-medium text-zinc-800 not-italic">{children}</span>,
